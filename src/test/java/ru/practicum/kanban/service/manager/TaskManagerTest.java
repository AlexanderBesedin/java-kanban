...
abstract class TaskManagerTest<T extends TaskManager> {
    ...
    protected int thisYear = LocalDateTime.now().getYear();
    ...
    void shouldUpdateTask() { //Default case
        ...
        assertEquals("TASK{id=1, name='name', description='description1', status=NEW, " +
    ...
    void shouldUpdateEpic() { //Default case
        ...
        assertEquals("EPIC{id=1, name='name', description='description1', status=NEW, subtasksIds=[], " +
    ...
    void shouldUpdateSubtask() { //Default case
        ...
        assertEquals("SUBTASK{id=2, name='name', description='description1', status=NEW, " +
                        "startTime=null, duration=null}",
                subtask.toString());
    }
    ...
    void shouldGetListTasks() {
        ...
        assertTrue(taskManager.getTasks().containsAll(tasks));
    }
    ...
    void shouldGetListEpics() {
        ...
        assertTrue(taskManager.getEpics().containsAll(epics));
    }
    ...
    void shouldGetListSubtasks() {
        ...
        assertTrue(taskManager.getSubtasks().containsAll(subtasks));
    }
    ...
    void shouldGetListPrioritizedTasksByTime() { // Default case
        ...
        Subtask task1 = new Subtask(//id#2
                name, description, 1,
                timeFormat.getFormatStartTime(String.format("12.05.%d 09:37", thisYear)),
                timeFormat.getFormatDuration("55")
        );
        taskManager.createSubtask(task1);
        Task task2 = new Task(//id#3
                name, description,
                timeFormat.getFormatStartTime(String.format("12.05.%d 10:30", thisYear)),
                timeFormat.getFormatDuration("90")
        );
        taskManager.createTask(task2);
        Task task3 = new Task(//id#4
                name, description,
                timeFormat.getFormatStartTime(String.format("12.05.%d 12:08", thisYear)),
    ...
    void shouldCreateTasksIfTimeNotOverlaps() { //Задачи не пересекаются, все задачи могут быть созданы
        Task task1 = new Task(
                name, description,
                timeFormat.getFormatStartTime(String.format("12.05.%d 12:00", thisYear)), timeFormat.getFormatDuration("60")
        );
        Task task2 = new Task(
                name, description,
                timeFormat.getFormatStartTime(String.format("12.05.%d 13:00", thisYear)), timeFormat.getFormatDuration("30")
        );
    ...
    @Test
        //Новая задача создается, если ее StartTime равен EndTime другой задачи
    void shouldCreateTaskIfStartTimeEqualPrevEndTime() {
        Task task1 = new Task(
                name, description,
                timeFormat.getFormatStartTime(String.format("12.05.%d 12:00", thisYear)),
                timeFormat.getFormatDuration("60")
        );
        Task task2 = new Task(
                name, description,
                timeFormat.getFormatStartTime(String.format("12.05.%d 13:00", thisYear)),
                timeFormat.getFormatDuration("30")
        );
    ...
    @Test
        //Между свободными StartTime и EndTime новой задачи есть другая задача
    ...
    @Test
        // StartTime эпика должен быть равен StartTime самой ранней подзадачи
    void shouldBeEpicStartTimeEqualStartTimeEarlySubtask() {
        Epic epic = makeEpic(name, description);
        Subtask subtask = new Subtask(
                name, description, epic.getId(),
                timeFormat.getFormatStartTime(String.format("12.05.%d 09:00", thisYear)),
                timeFormat.getFormatDuration("90")
        );
        taskManager.createSubtask(subtask);
        taskManager.createSubtask(
                new Subtask(
                        name, description, epic.getId(),
                        timeFormat.getFormatStartTime(String.format("12.05.%d 11:00", thisYear)),
                        timeFormat.getFormatDuration("90")
                )
        );
    ...
    @Test
        // Endtime эпика должен быть равен EndTime самой поздней подзадачи
    ...
    void shouldBeEpicDurationEqualSumDurationSubtasks() {
        ...
        taskManager.createSubtask(new Subtask(
                name, description, epic.getId(),
                timeFormat.getFormatStartTime(String.format("12.05.%d 14:00", thisYear)),
                duration2
        ));
    ...
    void shouldChangeTaskStatus() { //Default case
        ...
        taskManager.updateTaskStatus(task.getId(), status);
    ...
    void shouldNoChangeTaskStatusIfIncorrectId() { //Incorrect argument ID case
        ...
        taskManager.updateTaskStatus(2, expected);
    ...
    void shouldNoChangeTaskStatusWhoIsNull() { //Null argument Status case
        Task task = makeTask(name, description);
        taskManager.updateTaskStatus(task.getId(), null);
    ...
    void shouldChangeSubtaskStatus() { //Default case
        Subtask subtask = makeSubtask(name, description);
        Status status = Status.IN_PROGRESS;
        taskManager.updateSubtaskStatus(subtask.getId(), status);
    ...
    void shouldNoChangeSubtaskStatusIfIncorrectId() {  //Incorrect argument ID case
        Subtask subtask = makeSubtask(name, description);
        Status status = Status.IN_PROGRESS;
        taskManager.updateSubtaskStatus(3, status);
    ...
    void shouldNoChangeSubtaskStatusWhoIsNull() { //Null argument Status case
        Subtask subtask = makeSubtask(name, description);
        taskManager.updateSubtaskStatus(subtask.getId(), null);
    ...
    void shouldBeEpicStatusInProgressIfSubtasksAreDoneAndNew() { //Подзадачи со статусом DONE и NEW
        ...
        for (Subtask sub : taskManager.getSubtasks()) {
            taskManager.updateSubtaskStatus(sub.getId(), Status.DONE);
        }
        taskManager.updateSubtaskStatus(subtask.getId(), Status.NEW);
    ...
    void shouldClearSubtasks() {
        ...
        assertTrue(taskManager.getSubtasks().isEmpty() && !taskManager.getEpics().isEmpty());
    }
